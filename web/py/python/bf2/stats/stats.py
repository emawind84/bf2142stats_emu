...
class PlayerStat: 
	...
	def reset(self):
		...
		if g_debug: print "stats.py[210]: Resetting medals for player: " + str(self.playerId) + "(pid: " + str(self.profileId) + ")"
		self.medals = None
	...
	def reconnect(self, player):
		...
		if g_debug:
			print "stats.py[291]: Reattaching score object from old dead player %d to new player %d" % (self.localScore.index, player.index)
	...
	def copyPlayerData(self, player):
		...
		if g_debug: print "stats.py[364]: Trying to copy data for player: ", self.profileId
	
		#if host.ss_getParam('ranked'):
		if hasattr(player, 'medals'):
			if g_debug: print "stats.py[368]: Copy medals for player: ", self.profileId
			self.medals = player.medals
		else:
			if g_debug: print "stats.py[371]: Player had no medal stats. pid=", self.profileId
		#else:
...
def onPlayerConnect(player):
	...
	for stats in sessionPlayerStatsMap.itervalues():
		if connectingProfileId > 0 and connectingProfileId == stats.profileId:
			if g_debug: 
				print "stats.py[515]: Found old player record, profileId ", stats.profileId
			player.stats = stats
	...
	if not player.stats:
		...
		if g_debug: 
			print "stats.py[522]: Creating new record for player profileId ", connectingProfileId
...
def onPlayerKilled(victim, attacker, weapon, assists, object):
	...
	if attacker != None:
		...
		if attacker.getTeam() != victim.getTeam():
			...
			if g_debug: 
				print "stats.py[685]:  ", attacker.getName(), " streak = ", attacker.stats.currentKillStreakMedal, ", ", attacker.stats.currentKillStreakMedalSecond
...
def onVehicleDestroyed(vehicle, attacker):
		...
		if vehicle != None and attacker != None:
			...
			if g_debug: 
				print "stats.py[873]: Destroyed: ", attacker.stats.vehicles[vehicleType].destroyed
