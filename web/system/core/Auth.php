...
*/
    ...
    public static function IsValidSession()
    {
		...
		if($_SESSION['adminAuth'] != sha1($cfg->get('admin_user').':'.$cfg->get('admin_pass'))) 
			return false;
...
*/
    ...
    public static function Login($username, $password)
    {
		...
		if($username != $cfg->get('admin_user') || $password != $cfg->get('admin_pass')) 
		{
		...
		else 
		{
			...
			$_SESSION['adminAuth'] = sha1($cfg->get('admin_user') .':'. $cfg->get('admin_pass'));
			$_SESSION['adminTime'] = time();
			$SID = session_id();
			return TRUE;
		}
