...
file_put_contents('getbackendinfo.txt', $str);

if (isset($_SERVER["REMOTE_ADDR"]) AND $_SERVER["REMOTE_ADDR"] != "") {
	$clIP = $_SERVER["REMOTE_ADDR"];
} else {
	echo "You do not have permission to access this service.";
	exit;
}
$version = "1.7.94.0";
...
D\t".$timestamp."\t0\t".$clIP."\t".$clType."
H\tconfig
D\t";
if(!$isServer) {
	$Out .= "swiffHost.setLatestGameVersion ".$version."\n";
}

$Out .= 'rankSettings.setRank 0 0
